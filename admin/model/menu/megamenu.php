...
class ModelMenuMegamenu extends Model { 
	 ...
	 */ 
	public function delete( $id, $store_id = 0 ){
	...
	public function recursiveDelete($parent_id)
	{
		...
		$this->db->query($sql);
		$sql = " DELETE FROM ".DB_PREFIX ."megamenu WHERE megamenu_id=".(int)$parent_id .";";
		$this->db->query($sql);
		...
		if( $this->hasChild($parent_id) ){
			...
			foreach( $data as $menu ){
				if($menu['megamenu_id'] > 1) {
					 $this->recursiveDelete( $menu['megamenu_id'] );
				}	
			}
		}
	}
	...
	public function deletecategories(){
		...
		foreach ($categories as &$category){
			//delete megamenu description
			$query = $this->db->query("SELECT megamenu_id FROM ".DB_PREFIX."megamenu 
					WHERE `type`='category' AND
					type_submenu = 'menu' AND
					colums = 1 AND
					item=".$category['category_id']);
			if($query->num_rows){
				$megamenu_id = (int)$query->row['megamenu_id'];
				$sql3 = "DELETE FROM ".DB_PREFIX ."megamenu_description WHERE megamenu_id=".(int)$megamenu_id;
				$this->db->query( $sql3 );
			}
			//delete megamenu
			$sql1 = "DELETE FROM ".DB_PREFIX . "megamenu WHERE type_submenu = 'menu' AND `type` = 'category' AND colums = 1 AND item = {$category['category_id']}";
			$this->db->query( $sql1 );
		}
	}
	...
	public function importCategories(){
		...
		foreach ($categories as &$category){
			...
			if($this->checkExitItemMenu($category) == 0){
				...
				$this->insertCategory($category, $megamenu_parent_id);
			}
		} //die;
	}
	public function insertCategory($category = array(), $megamenu_parent_id){
		$data = array();
